...
// Zeitbudget des Hintergrund-Laufs. Netlify gibt Background-Functions bis zu
// 15 Minuten; das Budget bleibt trotzdem endlich, damit ein haengender Anbieter
// den Lauf nicht ewig offen haelt. Der frueher noetige Drahtseilakt entfaellt:
// Diese Function muss nicht mehr vor einem 26-Sekunden-Limit fertig sein, das
// Modell steht nicht mehr unter Zeitdruck, und die Mengen unten duerfen wieder
// nach fachlichen statt nach Uhr-Gruenden gewaehlt werden.
const SCAN_BUDGET_MS = Number(env("SCAN_BUDGET_MS")) || 240_000;
// Puffer fuer Serialisierung und das Schreiben des Ergebnisses.
const RESPONSE_RESERVE_MS = 5_000;
// Zeit, die nach der Modellauswertung dem Datumsabgleich vorbehalten bleibt.
const DATE_RESOLUTION_RESERVE_MS = 30_000;
// Obergrenze einer einzelnen Quellenaufloesung (zwei HTTP-Aufrufe).
const RESOLVE_MAX_MS = 10_000;
const MAX_DATE_RESOLUTIONS = 12;
...
export function resolveRegion(id: unknown): RegionConfig {
...
// -----------------------------------------------------------------------------
// Der eigentliche Lauf. Bewusst ohne HTTP: Diese Funktion weiss nichts von
// Requests, Kontingenten oder Blobs. Sie bekommt eine Region und liefert einen
// Lagebericht. Aufgerufen wird sie vom Hintergrund-Worker; der Ausloeser und
// die Statusabfrage kommen ohne sie aus. Genau diese Trennung erlaubt es, den
// Lauf laufen zu lassen, so lange er braucht, waehrend die Seite antwortet.
// -----------------------------------------------------------------------------
export async function runScan(regionId: unknown): Promise<any> {
  ...
  try {
    ...
    // Zurueck an den Worker, der das Ergebnis ablegt. Das Schreiben gehoert
    // nicht hierher: Diese Funktion soll rechnen, nicht verwalten.
    return result;
  } catch (error: any) {
    const message = error?.message || "Unbekannter Fehler bei der Analyse.";
    console.error("scan-core:", message);
