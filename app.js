  ...
  const runLiveScan = async region => {
    ...
    try {
      ...
      try {
        ...
        throw new Error(`Der Analyse-Dienst hat keine gültige JSON-Antwort geliefert (Status ${response.status}). ` + `Meist überschreitet die Live-Auswertung dann das Zeitlimit der Function oder das AI Gateway ` + `ist noch nicht aktiv (erst nach mindestens einem Produktions-Deployment). Bitte den Scan erneut starten.`);
      }
      if (!response.ok) {
