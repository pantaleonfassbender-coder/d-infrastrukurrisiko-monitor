...
/* GDELT queries a 30-day window across its whole article index: it needs
   far longer than the other feeds, drops connections under load and answers
   429 when it is busy. The 15-second budget aborted it on nearly every run,
   which left the news layer silently empty. The scan is a background
   function, so it can afford both a long wait and a few retries. */
const GDELT_TIMEOUT_MS = 60000;
const GDELT_ATTEMPTS = 3;
const GDELT_RETRY_DELAY_MS = [2000, 6000];
...
/* fetch() reports every network-level problem as a bare "fetch failed"; the
   useful part sits in err.cause. Naming it makes a run's error list readable
   instead of uniformly mysterious. */
function describeFetchError(err, url) {
  const code = err?.cause?.code || err?.code;
  const host = new URL(url).hostname;
  if (err?.name === "TimeoutError") return `Zeitüberschreitung bei ${host}`;
  if (code === "UND_ERR_CONNECT_TIMEOUT") return `Verbindungsaufbau zu ${host} überschritt das Zeitlimit`;
  if (code) return `${code} bei ${host}`;
...
export async function collectGdelt() {
  ...
  for (let attempt = 0; attempt < GDELT_ATTEMPTS; attempt += 1) {
    if (attempt > 0) await sleep(GDELT_RETRY_DELAY_MS[attempt - 1]);
    try {
      data = await fetchJson(url, { timeoutMs: GDELT_TIMEOUT_MS });
      break;
    } catch (err) {
      lastError = new Error(describeFetchError(err, url));
    }
  }
...
export async function collectNina() {
  ...
  const results = await Promise.allSettled(
    NINA_FEEDS.map(async ([source, url]) => {
      const items = await fetchJson(url).catch((err) => {
