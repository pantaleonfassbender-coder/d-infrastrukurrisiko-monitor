 ...
 *
 * Pass B carries a source obligation: `quelleUrl` is a required schema field
 * and must be one of the GDELT URLs handed to the model, so every incident on
 * the map is traceable to an article the collector itself retrieved. The
 * schema can only demand that the field is present — that its value really
 * comes from the GDELT list is checked in lib/core.mjs.
 *
      ...
      items: {
        ...
        properties: {
          ...
          quelleUrl: {
            type: "STRING",
            description:
              "PFLICHT: exakt eine URL aus Material B (GDELT), unverändert kopiert",
          },
        },
        required: ["date", "ort", "sektor", "schweregrad", "titel", "quelleUrl"],
      },
    },
  },
...
export async function structuredAnalysis(apiKey, { reportText, articles, warnings }) {
...
4. QUELLENPFLICHT: Jeder Vorfall MUSS in "quelleUrl" genau eine URL aus Material B tragen, Zeichen für Zeichen aus der Liste kopiert - keine gekürzten, geratenen oder aus dem Gedächtnis ergänzten URLs. Lässt sich ein Vorfall nicht mit einer URL aus Material B belegen, nimm ihn NICHT in "incidents" auf, auch wenn Material A oder C ihn nennt; erwähne ihn stattdessen nur im Text von "assessment".
...
MATERIAL B - GDELT-SCHLAGZEILEN (${articles.length}) - AUSSCHLIESSLICH DIESE URLS SIND ALS QUELLE ZULÄSSIG:
${articleList || "(keine)"}
