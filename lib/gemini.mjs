 ...
 * Lagebericht, the collected headlines (GDELT plus German publisher feeds)
 * and the NINA warnings and must answer in a fixed JSON schema — scores,
 * sector scores and geocodable incidents. The passes are separate because the
 * Gemini API does not allow combining the google_search tool with a JSON
 * response schema in one call.
 *
...
export async function structuredAnalysis(apiKey, { reportText, articles, warnings }) {
  const articleList = articles
    .map((a) => `- [${(a.date || "?").slice(0, 10)}] ${a.title} (${a.domain}) ${a.url}`)
    .join("\n");
...
B) Nachrichten-Schlagzeilen deutschsprachiger Medien (GDELT und die RSS-Feeds der großen deutschen Nachrichtenhäuser, letzte 30 Tage; das Datum in Klammern ist das Veröffentlichungsdatum),
...
MATERIAL B - NACHRICHTEN-SCHLAGZEILEN (${articles.length}):
${articleList || "(keine)"}
