 ...
 * Lagebericht, the GDELT articles and the NINA warnings and must answer in a
 * fixed JSON schema — scores, sector scores and geocodable incidents. The
 * passes are separate because the Gemini API does not allow combining the
 * google_search tool with a JSON response schema in one call.
 *
 * Credentials come from the caller: a Netlify function hands in the client
 * that resolveGemini() built from its environment, so this module never
 * reaches for process.env itself.
 */

/* Google's public endpoint. Netlify AI Gateway sets GOOGLE_GEMINI_BASE_URL
   to its own proxy instead, and a gateway key is only valid against that
   proxy — which is why the key and the base URL are resolved together in
   resolveGemini() and always travel as one client object. */
...
/* The credential and the endpoint it belongs to, resolved from an env-like
   object. Returns null when no Gemini access is configured at all. A key the
   operator set themselves wins; Netlify AI Gateway only fills the gap, and
   then its own base URL comes along with it. */
export function resolveGemini(env = {}) {
  const ownKey = (env.GEMINI_API_KEY || "").trim();
  if (ownKey) {
    return {
      apiKey: ownKey,
      baseUrl: trimSlash(env.GOOGLE_GEMINI_BASE_URL) || DEFAULT_API_BASE,
      origin: "GEMINI_API_KEY",
    };
  }
  const gatewayKey = (env.NETLIFY_AI_GATEWAY_KEY || "").trim();
  const gatewayBase = trimSlash(env.GOOGLE_GEMINI_BASE_URL);
  if (gatewayKey && gatewayBase) {
    return { apiKey: gatewayKey, baseUrl: gatewayBase, origin: "ai-gateway" };
  }
  return null;
}
...
/* A rejected credential cannot be retried away, and the raw Google wording
   ("API key not valid") is shown verbatim on the site — so it gets the one
   piece of context that actually resolves it: on Netlify, functions pick up a
   changed environment variable only through a new deploy. */
function credentialError(client, model, status, detail) {
  const source =
    client.origin === "ai-gateway"
      ? "Netlify AI Gateway"
      : "GEMINI_API_KEY (Netlify-Umgebungsvariable)";
  return new Error(
    `${model}: HTTP ${status} ${detail} — Gemini hat den Zugang aus ${source} abgelehnt. ` +
      "Wert prüfen und danach neu deployen: Functions übernehmen geänderte " +
      "Umgebungsvariablen erst mit dem nächsten Deploy."
  );
}

const CREDENTIAL_STATUS = new Set([400, 401, 403]);

async function callModel(client, payload) {
  let lastError = null;
  for (const model of MODEL_CANDIDATES) {
    const res = await fetch(`${client.baseUrl}/models/${model}:generateContent`, {
      method: "POST",
    ...
    if (!retriable && CREDENTIAL_STATUS.has(res.status) && /key|credential|permission|auth/i.test(detail)) {
      throw credentialError(client, model, res.status, detail);
    }
...
export async function groundedReport(client) {
...
Schreibe einen sachlichen Lagebericht auf Deutsch (max. 400 Wörter), ohne Markdown-Überschriften.`;
  ...
  const { model, text, data } = await callModel(client, {
    contents: [
      {
        parts: [
          {
...
export async function structuredAnalysis(client, { reportText, articles, warnings }) {
...
B) Nachrichten-Schlagzeilen aus GDELT (deutschsprachige Medien, letzte 30 Tage),
...
MATERIAL B - GDELT-SCHLAGZEILEN (${articles.length}):
${articleList || "(keine)"}
...
Heutiges Datum: ${new Date().toISOString().slice(0, 10)}. Erstelle das strukturierte Lagebild.`;
  ...
  const { model, text } = await callModel(client, {
